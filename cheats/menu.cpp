...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\Angolao\\Configs\\");
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\Angolao\\Scripts\\");
...
void c_menu::render2(bool is_open) {
	...
	Style->Colors[ImGuiCol_WindowBg] = ImColor(0, 128, 255, 0);
	Style->Colors[ImGuiCol_ChildBg] = ImColor(238, 130, 238);
...
// IMPORTANT: DO TAB SWITCHING BY LAST_TAB!!!!!
	...
	if (ImGui::Begin(crypt_str("AngolaOnTop"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
	{
		...
		ImGui::BeginChild("##Full", ImVec2{ 720, 560 }, false);
		{
			...
			ImGui::BeginChild("##Up", ImVec2{ 720, 50 }, false);
			{
				...
				ImGui::PushFont(c_menu::get().futura_large); ImGui::TextColored(ImColor(255, 255, 255), "Angola"); ImGui::PopFont();
				#if BETA
				ImGui::SetCursorPos(ImVec2{ 90, 25 });
				ImGui::PushFont(c_menu::get().futura); ImGui::TextColored(ImColor(255, 255, 0), "DEV"); ImGui::PopFont();
				#endif

				ImGui::SetCursorPos(ImVec2{ 240, 0 });
				if (ImGui::Tab("1", "HVH", "Angola", ImVec2(85, 50), Active_Tab == 1 ? true : false))
					Active_Tab = 1;

				ImGui::SetCursorPos(ImVec2{ 335, 0 });
				if (ImGui::Tab("2", "Legit", "Hook", ImVec2{ 85, 50 }, Active_Tab == 2 ? true : false))
					Active_Tab = 2;
				...
				if (ImGui::Tab("4", "Skins", "Models", ImVec2{ 85, 50 }, Active_Tab == 4 ? true : false))
					Active_Tab = 4;

				ImGui::SetCursorPos(ImVec2{ 620, 0 });
				if (ImGui::Tab("5", "Misc", "Scripts", ImVec2{ 85, 50 }, Active_Tab == 5 ? true : false))
					Active_Tab = 5;
			}
			ImGui::EndChild();
					...
					if (Active_Tab == 1 && Active_Rage_Tab == 1) //Rage | General
					{
						...
						ImGui::BeginChild("it3ems", { 300, 430 });
						{
							...
							ImGui::BeginGroup();
							{
								...
								ImGui::Checkbox(crypt_str("Bruteforce resolver"), &g_cfg.ragebot.pitch_antiaim_correction);
								...
								if (g_cfg.ragebot.double_tap)
								{
									...
									static auto myp2e = 0;
									draw_combo(crypt_str("Double tap mode"), g_cfg.ragebot.dt_mode, DTSpeed, ARRAYSIZE(DTSpeed));

									if (g_cfg.ragebot.dt_mode == 0)
									{
										g_cfg.ragebot.dt_shift = 11;
									...
									ImGui::Checkbox(crypt_str("No teleport"), &g_cfg.ragebot.slow_teleport);
								}
						...
						ImGui::BeginGroup();
						{
								...
								if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(220 * dpi_scale, 26 * dpi_scale)))
								{
									...
									auto get_dir = [&folder]() -> void
									{
										...
										if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
											folder = std::string(path) + crypt_str("\\Angolao\\Scripts\\");
...
void c_menu::create_style()
{
	...
	_style.Colors[ImGuiCol_::ImGuiCol_Header] = ImColor(255, 16, 16, 180);
	_style.Colors[ImGuiCol_::ImGuiCol_HeaderHovered] = ImColor(16, 255, 16, 180);
	_style.Colors[ImGuiCol_::ImGuiCol_HeaderActive] = ImColor(16, 255, 16, 255);
	_style.Colors[ImGuiCol_::ImGuiCol_TitleBg] = ImColor(16, 16, 16, 255);
	_style.Colors[ImGuiCol_::ImGuiCol_TitleBgActive] = ImColor(16, 255, 16, 180);
	_style.Colors[ImGuiCol_::ImGuiCol_TitleBgCollapsed] = ImColor(16, 16, 16, 180);
	_style.Colors[ImGuiCol_::ImGuiCol_WindowBg] = ImColor(16, 16, 255, 180);
	_style.Colors[ImGuiCol_::ImGuiCol_ChildWindowBg] = ImColor(30, 30, 30, 255);
	_style.Colors[ImGuiCol_::ImGuiCol_ChildBg] = ImColor(30, 30, 255, 255);
