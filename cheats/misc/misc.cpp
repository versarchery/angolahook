...
#if RELEASE
#if BETA
#define VERSION crypt_str("AngolaHook [beta] | ")
#else
#define VERSION crypt_str("AngolaHook | ")
#endif
#else
#define VERSION crypt_str("AngolaHook [alpha] | ")
#endif
int pre_flags = 0;
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("https://discord.gg/6fergEtWNG"),
		crypt_str("Get AngolaHook and stop losing."),
		crypt_str("AngolaHook tapping your broken v3 rofl"),
		crypt_str("Get Good. Get AngolaHook."),
	};
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	static auto recharge_double_tap = g_cfg.ragebot.recharge_dt;
	...
	if (!g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && g_ctx.send_packet && (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 && g_ctx.globals.weapon->is_knife())) //-V648
	{
		...
		util::copy_command(user_cmd, g_cfg.ragebot.dt_shift - g_cfg.ragebot.dt_tolerance);
